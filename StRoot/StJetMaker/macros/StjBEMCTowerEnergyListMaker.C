...
class StjBEMCTowerEnergyListMaker : public StMaker {

public:
  
  StjBEMCTowerEnergyListMaker(const Char_t *name, TDirectory* file, StMuDstMaker* uDstMaker)
  : StMaker(name), _file(file), _uDstMaker(uDstMaker) { }
  virtual ~StjBEMCTowerEnergyListMaker() { }

  const char* GetCVS() const
  {static const char cvs[]="Tag $Name: SL18c $ $Id: StjBEMCTowerEnergyListMaker.C,v 1.3 2014/08/06 11:43:24 jeromel Exp $ built " __DATE__ " " __TIME__; return cvs;}
...
public:
  ...
  ClassDef(StjBEMCTowerEnergyListMaker, 1)
};
