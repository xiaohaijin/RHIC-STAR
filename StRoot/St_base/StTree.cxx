...
const char *TFOPT[9] = {"0",    "READ",     "RECREATE", "UPDATE", "0",
                        "READ", "RECREATE", "UPDATE",   0};
...
TObject *StIO::Read(TFile *file, const char *name) {
  ...
  if (name[0] == '*') {
    ...
    key = (TKey *)gDirectory->GetListOfKeys()->FindObject(name);
...
TObject *StIO::Read(TFile *file, const StUKey &ukey) {
  ...
  retn = event->fObj;
  assert(!retn || retn == (TObject *)(-1) ||
         event->GetUniqueID() == ukey.GetSum());
  delete event;
  return retn;
}
//_______________________________________________________________________________
Int_t StIO::GetNextKey(TFile *file, StUKey &ukey, ULong_t &handle) {
  ...
  TString tk = ukey.GetKey();
  ukey = kUMAX;
  const char *prevkey = (const char *)tk;
  int lname = strlen(ukey.GetName()) + 1;

  lk = file->GetListOfKeys();
  if (!lk) return 1;
  if (!lk->TestBit(kSorted)) {
    lk->Sort();
    lk->SetBit(kSorted);
    handle = 0;
  }
...
//_______________________________________________________________________________
TFile *StIO::Open(const char *name, Option_t *option, const char *title,
                  Int_t compress) {
...
//_______________________________________________________________________________
StBranch::StBranch(const char *name, StTree *parent, Option_t *opt)
    : TDataSet(name, parent) {
...
Option_t *StBranch::GetIOMode() {
  IOMODE[0] = 0;
  if (fIOMode > 0) IOMODE[0] = RWU[int(fIOMode)];
  return IOMODE;
}
...
Int_t StBranch::UpdateFile(const char *file) {
  ...
  if (strncmp(".none ", GetFile(), 6) ==
      0) {  // FileName was corrupted by old bug
...
//_______________________________________________________________________________
void StBranch::Clear(Option_t *) { Delete(); }
//===============================================================================

ClassImp(StTree)

//_______________________________________________________________________________
StTree::StTree(const char *name) : StBranch(name) { SetTitle(".StTree"); }
...
void StTree::Clear(Option_t *) {
  TDataSetIter next(this);
  StBranch *br;
  while ((br = (StBranch *)next())) {/*if(!br->IsOption("const"))*/
    br->Clear();
  }
}
...
void StTree::Close(const char *opt) {
  ...
  while ((br = (StBranch *)next())) {  // branch loop
    ...
    if (br->fIOMode & 2) {
      br->Open();
      TFile *tfbr = br->GetTFile();
      if (!tfbr) continue;
      if (tfbr->IsWritable()) {
        TDataSet *par = GetParent();
        SetParent(0);
...
//_____________________________________________________________________________
Int_t StFile::GetNBundles() { return fDS->GetListSize(); }
//_____________________________________________________________________________
Int_t StFile::GetNFiles() { return fDS->GetListSize(); }
//_____________________________________________________________________________
...
Int_t StFile::AddEvent(UInt_t r, UInt_t e) {
  TObjectSet *dsfam = (TObjectSet *)fDS->Last();
  if (!dsfam) return 1;
  TDataSet *dsfil = dsfam->Last();
  if (!dsfil) return 1;

  TDataSet *dskey = (TDataSet *)dsfam->GetObject();
  if (!dskey) {
    dskey = new TDataSet("uklist");
    dsfam->SetObject(dskey, 1);
  }
  char cbuf[40];
...
void StFile::lsFull(Option_t *opt) {
  ...
  while (!GetNextBundle()) {               // Bundle loop
    for (int idx = 0; idx < 999; idx++) {  // File loop
      ...
      printf("%4d BR=%-7s NK=%-4d NR=%-4d SZ=%8.2fM File= %s\n", numFile,
             br.Data(), nkeys, nreks, dsize, basname.Data());
      delete tf;
  ...
  while ((cat = (Cat *)nextBr())) {
    ...
    printf("%4d BR=%-10s NK=%-4d NR=%-4d SZ=%8.2fM NFiles %4d\n", ibr,
           cat->GetName(), cat->fNKeys, cat->fNRecs, cat->fSize * 1.e-6,
           cat->fNFiles);
  }
  blist.Delete();
  fIter = savIter;
